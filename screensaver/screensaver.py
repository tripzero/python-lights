...
def larsonScanner2():
    ...
    while True:
        ...
        for l in range(length):
            ...
            leds.transformColorTo(
                led + l, (255, 0, 0), 0.001).then(leds.transformColorTo, led + l, (0, 0, 0), 0.001)
        ...
        for l in range(length):
            ...
            leds.transformColorTo(
                led2 - l, (255, 0, 0), 0.001).then(leds.transformColorTo, led2 - l, (0, 0, 0), 0.001)
...
def randomRainbowTransforms():
    ...
    for i in range(leds.ledArraySize):
        concurrentTransform.addAnimation(
            leds.transformColorTo, i, (r, g, b), 1000)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num', dest="numLeds",
                        help="number of leds", type=int, default=1)
    parser.add_argument('--fps', dest="fps",
                        help="frames per second", type=int, default=5)
    parser.add_argument('--larson', dest="larson",
                        help="do larson animation in a loop", action='store_true')
    parser.add_argument('--rainbow', dest="rainbow",
                        help="do rainbow wave animation in a loop", action='store_true')
    parser.add_argument('--driver', dest="driver",
                        help="driver to use", default=None)
    parser.add_argument('address', help="address",
                        default="localhost", nargs="?")
    parser.add_argument('port', help="port", default=1888, nargs="?")
    parser.add_argument('--device', type=str, dest="device_name",
                        default="", help="particle device name")
    parser.add_argument('--config', type=str, dest="config_name",
                        default="config.json", help="config")
    ...
    if args.driver:
        ...
        if not Driver:
            raise Exception("{} driver not available.  Installed drivers: {}".format(
                config['driver'], ", ".join(photons.drivers)))
